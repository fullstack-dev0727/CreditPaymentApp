...
+ (void)renderSizeDependentProperties:(UINavigationBar*)bar
{
    ...
    if ([NUISettings hasProperty:@"background-color-top" withClass:className]) {
        ...
        {
            CGRect frame = bar.bounds;
            UIImage *colorImage = [NUIGraphics colorImage:[NUISettings getColor:@"background-color" withClass:className] withFrame:CGRectMake(0, 0, frame.size.width, frame.size.height+20)];
